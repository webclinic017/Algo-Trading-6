...
class Scalper:
    def __init__(self):
        ...
        self.update_data(self.instrument)
        self.run = False
        self.rr_mult = 1.5
        self.order = None

    def update_data(self, instrument):
        m5 = api.get_candlestick_data(instrument,30,"M5")
        h1 = api.get_candlestick_data(instrument,30,"H1")
            ...
            while self.run:
                if dt.datetime.now().second == 0  and len(api.get_open_positions()['positions']) == 0:
                    ...
                    for instrument in self.instruments:
                        m5,h1 = self.update_data(instrument)
                        print(f'\t{m5.index[-1]}')
                        c = m5['Close'][-1]
...
class CipherB:
    ...
    def start(self):
        ...
        try:
            ...
            while self.run:
                if dt.datetime.now().minute % 15  == 1 and len(api.get_open_positions()['positions']) == 0 and len(api.get_orders()['orders']) == 0:
                    self.update_data()
                    print(f'\n---- {time.asctime()}: Looking for Signals ---- \n')
                    df = self.data
                    print(f'\t{df.index[-1]}')
            ...
            while self.run:
                if dt.datetime.now().minute % 15  == 1 and len(api.get_open_positions()['positions']) == 0 and len(api.get_orders()['orders']) == 0:
                    ...
                    if len(api.get_orders()['orders']) == 0 and df['Buy'][-1]:
                        ...
                        qty = api.calculate_qty(c,stop_loss,"EUR_USD",balance=api.get_balance())
                        api.market_order(self.instrument,qty,trailing_stop=c-stop_loss,take_profit=take_profit)
                        alert("Long Signal","Cipher B 5 Long","8452697031@mms.att.net")
                        print("\tLong Signal Detected")
                    elif len(api.get_orders()['orders']) == 0 and df['Sell'][-1]:
                        stop_loss = c + self.atr_mult * df['ATR'][-1]
                        take_profit = c - self.rr_mult*(stop_loss-c)
                        qty = api.calculate_qty(c,stop_loss,"EUR_USD",balance=api.get_balance())
                        api.market_order(self.instrument,qty,trailing_stop=stop_loss-c,take_profit=take_profit)
                        alert("Short Signal","Cipher B 5 Short","8452697031@mms.att.net")
                        print("Short Signal Detected")
                    else:
                        print('\tNo Signals found')
            print('\nStopping Cipher B Algorithm...')
